...
@Slf4j
@Service
class GradlewGenerator implements Generator {

    void execute(Context context, ZipOutputStream zos) {

        String template = new ClassPathResource("generators/gradlew.txt").inputStream.text

        ZipEntry zipEntry = new ZipEntry("gradlew")
